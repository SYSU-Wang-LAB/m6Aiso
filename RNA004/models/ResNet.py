...
class multi_modification_Net(nn.Module):
	def __init__(self,dropout_probility,d_model,number_of_classes):
		super(multi_modification_Net,self).__init__()
		#################################################
		self.sequence_conv = nn.Sequential(
			nn.Conv1d(in_channels=10,out_channels=2,kernel_size=9,padding=0,bias=False),
			nn.Tanh()
			)
		#################################################
		...
		self.conv_layer_2 = nn.Sequential(
			nn.Conv2d(in_channels=d_model,out_channels=d_model*2,kernel_size=3,padding=1,bias=False),
			nn.BatchNorm2d(d_model*2),
			nn.Tanh(),
			nn.Dropout(p=dropout_probility,inplace=False)
			)
		self.conv_layer_3 = nn.Sequential(
			nn.Conv2d(in_channels=d_model*2,out_channels=d_model,kernel_size=3,padding=1,bias=False),
			nn.BatchNorm2d(d_model),
			nn.Tanh(),
			nn.Dropout(p=dropout_probility,inplace=False)
			)
...
if __name__=="__main__":
	sequence = torch.zeros((255,10,11))
	signal = torch.zeros((255,10,3))
	model = multi_modification_Net(dropout_probility=0.2,d_model=32,number_of_classes=3)
	out = model(
